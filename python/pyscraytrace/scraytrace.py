...
class scraytrace:
    ...
    def __init__(self,
        ...
        if obslonlat is not None:
            obspos = obslonlat[2] * [np.sin(obslonlat[1]),
...
class TestScraytrace(unittest.TestCase):
    ...
    def test_CME(self):
        ...
        rt.raytrace()
        rt.dispim(cmap='gray', minmax=(1e-15, 1e-9))
...
#        neang = np.deg2rad([80, 0, 3.8])  # Parameters work on COR2 and HI
        ...
        sim1 = scraytrace(losnbp=200, modelid=modelid, neang=neang, physics=5)
...
#                    windowXYPos=(1100, 50))
    ...
    # ------------------------------------------------------------------------
        ...
        sim1 = scraytrace(losnbp=100, modelid=79, neang=neang,
